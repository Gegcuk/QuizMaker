...
- After selecting an issue as the next implementation candidate, pause before changing source code, configuration, migrations, CI, or infrastructure. Explain in plain language the verified current behavior and why it is a problem, the proposed outcome and how it would work, and at least one realistic before-and-after example. If more than one sound implementation or behavior is possible, present the meaningful options and tradeoffs and state a recommendation. Wait for the repository owner to confirm that the problem is real and that the proposed solution should be implemented; issue text, labels, priority, or roadmap placement alone are not approval.
- Before selecting or changing any product or operational policy—including retry counts, provider-attempt limits, fallback order or eligibility, scheduling or fairness rules, billing or charging semantics, security, permissions, ownership, retention, or comparable user-visible behavior—obtain explicit approval from the repository owner in the current task. Do not infer approval from existing code, an issue or roadmap description, or an unrelated earlier decision. Approval is not required for a fix that demonstrably preserves the current policy.
- Ask for that approval in a clear, human-readable question with 2–3 mutually exclusive options, the consequence of each option, a recommended option with its rationale, and at least one realistic user or operator example. Do not implement the policy change until the owner selects an option or states an equivalent decision.
- Keep each task and prospective pull request independently understandable, testable, and reviewable. Prefer no more than 1,000 added handwritten lines across source, tests, configuration, and documentation. This is a non-strict reviewability guideline, not a hard limit, completion gate, or automatic reason to split a coherent change.
- When a change grows beyond the preferred size, split it only when each vertical slice is independently useful, backward compatible, and verifiable. A larger coherent change may remain together when splitting would reduce clarity or safety; briefly explain that judgment and exclude unrelated work. Never create incomplete class-by-class slices merely to meet a line count.
