...
- CI preloads Maven dependencies, then runs `verify` in Maven offline mode inside a Linux network namespace with no default route. Only namespace loopback port `3306` is bridged to the CI MySQL service. The Actions runner remains online so logs and artifacts can still be uploaded.
