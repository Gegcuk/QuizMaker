...
- CI preloads Maven dependencies and performs a no-test `test-compile` to cache dependencies declared only in compiler `annotationProcessorPaths`. It then runs `clean verify` in Maven offline mode inside a Linux network namespace with no default route, proving a clean compilation without provider access. Only namespace loopback port `3306` is bridged to the CI MySQL service. The Actions runner remains online so logs and artifacts can still be uploaded.
