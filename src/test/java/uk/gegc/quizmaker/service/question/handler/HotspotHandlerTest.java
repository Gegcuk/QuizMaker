...
class HotspotHandlerTest {
    ...
    record FakeReq(JsonNode content) implements QuestionContentRequest {
        @Override public QuestionType getType()    { return QuestionType.HOTSPOT; }
        @Override public JsonNode     getContent() { return content;               }
    }

    @Test void validSingleRegion_doesNotThrow() throws Exception {
    ...
    @Test void missingImageUrl_throws() throws Exception {
        JsonNode p = mapper.readTree("""
    ...
    @Test void blankImageUrl_throws() throws Exception {
        JsonNode p = mapper.readTree("""
    ...
    @Test void missingRegions_throws() {
        JsonNode p = mapper.createObjectNode().put("imageUrl","http://x");
        assertThrows(ValidationException.class,
                () -> handler.validateContent(new FakeReq(p)));
    }

    @Test void emptyRegions_throws() throws Exception {
        JsonNode p = mapper.readTree("""
    ...
    @Test void regionMissingField_throws() throws Exception {
        JsonNode p = mapper.readTree("""
    ...
    @Test void regionNonIntField_throws() throws Exception {
        JsonNode p = mapper.readTree("""
